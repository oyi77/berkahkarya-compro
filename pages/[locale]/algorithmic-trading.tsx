...
export default function AlgorithmicTrading({ locale }: { locale: Locale }) {
  const d = productsFullData['algorithmic-trading'][locale];
  const pr = d.pricing.map(t => ({ ...t, features: [...t.features], cta: { ...t.cta } }));
  return (
    <Layout title={d.meta.title} description={d.meta.description}>
      <HeroSection eyebrow={d.hero.eyebrow} title={d.hero.title} description={d.hero.description} buttons={[...d.hero.buttons]} dark character={characters['algorithmic-trading']} />
      ...
      <TestimonialSection title={locale === 'id' ? 'Kata Trader yang Sudah Pakai Sistem Ini' : 'Traders Who Use This System'} items={[...d.testimonials]} />
